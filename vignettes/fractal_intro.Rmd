...
This vignette is a brief overview of fractal dimension methods in `habtools`. There are currently five methods:

| Method | Function | Data | Default `lvec` | Notes |
| :---- | :---- | :---- | :---- | :---- |
| Height variation | `hvar` | RasterLayer | 10 x res to extent | Invariant to height. Sensitive to non-fractal edges and outliers. |
| Standard deviation | `sd` | RasterLayer | 10 x res to extent | Invariant to height. Sensitive to non-fractal edges and outliers. |
| Area | `area` | RasterLayer or mesh3d | res to 1/8 extent | Sensitive to height; approaches 2 for most terrains. Less sensitive to non-fractal anomalies. |
| Cube counting | `cubes` | RasterLayer or mesh3d | 2 x res to extent | Sensitive to height; approaches 2 for most terrains. More reliable for closed 3D object. Less sensitive to non-fractal anomalies. |
| Box counting | `boxes` | xy data.frame | 10 x res to extent | For 2D shapes only. Less sensitive to non-fractal anomalies. |
...
Selecting a sensible sequence of scales (i.e., the length vector `lvec`) is therefore critical. The `lvec` should range between the resolution and extent of the surface or mesh, or across a range of scales relevant to the question being address. If you do not enter an `lvec`, each function will estimate one for you based on some rules of thumb for that particular method. For example, `hvar`, `sd`, `cubes` and `boxes` will generate a sequence from 2- to 10-times the resolution to the extent on a log2-scale. `area` will generate a sequence from the resolution to 1/8 the extent on the log2-scale. 
