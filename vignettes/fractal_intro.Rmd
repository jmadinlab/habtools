...
In this vignette, we give a brief overview of fractal dimension methods in `habtools`. 

There are currently five fractal dimension methods in `habtools`: `hvar`, `sd`, `area`, `cubes` and `boxes`. The first four can be applied to DEMs. `area` and `cubes` can be applied to meshes. `boxes` can only be applied to 2D outlines. 
...
Generally, fractal dimensions are estimated by measuring values (e.g., counts of cubes, heights, areas) across a sequence of different resolutions or length scales, and then finding the slope between these scales and the measured values. Therefore, selecting a sensible sequence of scales (i.e., the length vector `lvec`) is critical, and can differ among methods. If you don't enter an `lvec`, the function will estimate one for you based on some rules of thumb.

For example, 
```{r}
# simulate fractal terrain
surf <- sim_dem(n=128, smoothness=0.5)
