...
In this vignette, we highlight several issues that may arise when using `habtools` for estimating fractal dimension. We apply three methods (cube counting, height variation and area method) to a series of simulated and natural surfaces. We highlight that each method has inherent biases and should be used cautiously. For example, some methods are sensitive to particular elements, like outliers, z-scaling or drop-offs, while others are invariant to them. We note that the cubes method is more appropriately applied to fully 3D meshes, rather than surfaces that tend to be greater in extent than in height and continuous rather than watertight objects. Nonetheless, the comparisons are still useful. 
