...
```{r}
# Height variation method
fd(dem1, method = "hvar", lvec = c(0.125, 0.25, 0.5, 1, 2), plot = TRUE)
```

We can visualize the calculation and model fit by setting `diagnose=TRUE`. 
...
```{r}
fd(dem1, method = "hvar", lvec = c(0.125, 0.25, 0.5, 1, 2), diagnose = TRUE)
```
